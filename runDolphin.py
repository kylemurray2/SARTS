...
def cmdLineParser():
    ...
    parser.add_argument('-p','--do_ps', action='store_true', help='Run PS estimation.') 
    parser.add_argument('-s','--sequential_PL', action='store_true', help='Run sequential_PL function.')
    
    return parser.parse_args()


def doPS(ps,vrt_stack,logger):
    '''
    Make PS/ps_pixels
    '''

    ps.ps_output.parent.mkdir(parents=True, exist_ok=True)

    if ps.ps_output.exists():
        logger.info(f"Skipping making existing PS file {ps.ps_output}")
    else:
        logger.info(f"Creating persistent scatterer file {ps.ps_output}")
        existing_amp = None
        existing_disp = None
        ...
        dolphin_ps.create_ps(
            ...
            output_amp_dispersion_file=ps.amp_dispersion_file,
...
def seqPL(ps,input_file_list,logger):
    ...
    ministack_planner = stack.MiniStackPlanner(
        ...
        max_num_compressed=ps.max_num_compressed,
    ...
    if len(phase_linked_slcs) > 0:
        ...
        (
            ...
            ps_mask_file=ps.ps_output,
            amp_mean_file=ps.amp_mean_file,
            amp_dispersion_file=ps.amp_dispersion_file,
